...
def run_experiment(model,
  ...
  print('\nThe fine-tunned model continued your prompts like this: ')
  for j in range(len(samples)):
      print('\n{}'.format(samples[j]))

  print("\nPrinting missalignment statistics")
...
class Experiment:
  ...
  def generate_samples(
    ...
    conditions = conditions
    print('\nThe conditional token applied is: {}'.format(conditions))
    prompts_and_conditions = [prompts[i]+conditions[0] for i in range(len(prompts))] # Concatenate all prompts and conditions 
    documents = []
  ...
  def split_documents_to_sentences(self, documents):
    ...
    for text in documents:
      ...
      condition = [self.conditions[0]] * output_length
