...
import random
from scripts.Experiment2 import Experiment, run_experiment, create_conditions, create_prompts
...
def load_and_run(input_params, random_seed, prompt):
  ...
  #Define prompts
  prompts = create_prompts(texts, prompt_source, number_of_samples, prompt=prompt)
  if prompt_source == 1:
    prompts = [string[:-13] for string in prompts]
  print('\nThese are your input prompts: \n{}'.format(prompts))
  ...
  file_name = path_output_table + "/Table" + "_numsamples=" + str(number_of_samples)+ "_model=" + model_all_names + "_condtox=" + condtoxic_all +"_condsent=" + condsent_all +"_prompt=" + str(prompt_source)+ "p=" +str(top_p)+ "k=" +str(top_k)+ "temp=" +str(temperature)+ "ngram=" +str(no_repeat_ngram_size)+".csv"
  df = pd.concat(tables_list, axis=0, ignore_index=True)
